...
export function Section15Hero({ locale = "uz" }: { locale?: string }) {
  const text = copy[locale as keyof typeof copy] ?? copy.uz;
  ...
  return (
    <section className="gurman-orbit-hero" aria-labelledby="gurman-orbit-title">
      ...
      <header className="gurman-orbit-hero__nav">
        <Link className="gurman-orbit-hero__brand" href={`/${locale}`} aria-label="Manzil home"><span aria-hidden="true" className="gurman-orbit-hero__arch" /><span>Manzil</span></Link>
        <Link className="gurman-orbit-hero__catalogue" href={`/${locale}/discover`}>{text.nav} <span aria-hidden="true">↗</span></Link>
      </header>
