...
// thirty-seven; this is the only part that is "nest".
function frame(t: number, P: Params, out: Dot[]) {
        ...
        // Inner shell: the other way, swelling out to meet it. Both land in one
        // depth sort, so an inner dot can paint over an outer one.
...
export default function OrbNest(props: Props) {
    const {
        style,
        dotColor = "#F4F1EA",
        accentColor = "#00FFCB",
    ...
    useEffect(() => {
        ...
        if (!ctx) {
            console.error("OrbNest: 2D context unavailable")
            return
        }
