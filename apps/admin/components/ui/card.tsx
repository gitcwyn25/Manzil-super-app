...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div ref={ref} className={cn("rounded-[10px] border border-border bg-card text-card-foreground shadow-[0_1px_2px_rgba(18,36,40,0.03)]", className)} {...props} />
  )
);
...
const CardTitle = React.forwardRef<HTMLHeadingElement, React.HTMLAttributes<HTMLHeadingElement>>(
  ({ className, ...props }, ref) => <h3 ref={ref} className={cn("text-sm font-semibold tracking-[-0.01em]", className)} {...props} />
);
